...
const converterStingParaData = (dataString) => {
...
const api = {
  ...
  async salvarPensamento(pensamento) {
    try {
      const data = converterStingParaData(pensamento.data);
  ...
  async buscarPensamentosPorTermo(termo) {
    try {
      ...
      const termoEmMinusculos = termo.toLowerCase();

      const pensamentosFiltrados = pensamentos.filter((pensamento) => {
        return (
          pensamento.conteudo.toLowerCase().includes(termoEmMinusculos) ||
          pensamento.autoria.toLowerCase().includes(termoEmMinusculos)
        );
      });
